...
MNEMONIC_VAL="clock post desk civil pottery foster expand merit dash seminar song memory figure uniform spice circle try happy obvious trash crime hybrid hood cushion"
MNEMONIC_TEST_1="banner spread envelope side kite person disagree path silver will brother under couch edit food venture squirrel civil budget number acquire point work mass"
MNEMONIC_TEST_2="veteran try aware erosion drink dance decade comic dawn museum release episode original list ability owner size tuition surface ceiling depth seminar capable only"
MNEMONIC_RLY="alley afraid soup fall idea toss can goose become valve initial strong forward bright dish figure check leopard decide warfare hub unusual join cart"
...
$BINARY add-genesis-account $($BINARY --home $NODE_HOME/$CHAIN_ID keys show validator --keyring-backend test -a) 100000000000uchora  --home $NODE_HOME/$CHAIN_ID
$BINARY add-genesis-account $($BINARY --home $NODE_HOME/$CHAIN_ID keys show test-1 --keyring-backend test -a) 100000000000uchora  --home $NODE_HOME/$CHAIN_ID
$BINARY add-genesis-account $($BINARY --home $NODE_HOME/$CHAIN_ID keys show test-2 --keyring-backend test -a) 100000000000uchora  --home $NODE_HOME/$CHAIN_ID
$BINARY add-genesis-account $($BINARY --home $NODE_HOME/$CHAIN_ID keys show relayer --keyring-backend test -a) 100000000000uchora  --home $NODE_HOME/$CHAIN_ID

echo "Creating and collecting gentx..."
$BINARY gentx validator 7000000000uchora --home $NODE_HOME/$CHAIN_ID --chain-id $CHAIN_ID --keyring-backend test
...
sed -i -e "s/stake/uchora/g" $NODE_HOME/$CHAIN_ID/config/genesis.json
